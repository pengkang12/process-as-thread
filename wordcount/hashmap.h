...
public:
  ...
  void insert(const KeyType& key, size_t keylen, ValueType value) {
    if(_initialized != true) {
      PRINT("process %d: initialized at  %p hashmap is not true\n", getpid(), &_initialized);
    }
...
private:
  ...
  void insertEntry(struct HashEntry* head, const KeyType& key, size_t keylen, ValueType value) {
    ...
    head->count++;
    fprintf(stderr,"insertEntry entry %p at head %p, headcount %ld\n", entry, head, head->count);
    fprintf(stderr,"insertEntry entry %p, entrynext %p, at head %p hear->list %p headlist->next %p\n", entry,
...
public:
  ...
  iterator begin() {
    ...
    // fprintf(stderr,"in the beginiing of begin\n");
    // Get the first non-null entry
    while(pos < _buckets) {
      head = getHashEntry(pos);
      // fprintf(stderr,"begin, head %p pos %ld head->count %ld\n", head, pos, head->count);
      if(head->count != 0) {
        // Now we can return it.
        entry = (struct Entry*)head->getFirstEntry();
        //	fprintf(stderr,"In begin() function, head %p, firstentry %p, firstentry next %p\n", head, entry,
        //entry->list.next);
